...
def design_ecg_filter(fs=256, lowcut=0.5, highcut=100, order=4):
    """
    Design a Butterworth bandpass filter for ECG signal at 256 SPS
...
def apply_smoothing_filter(signal, window_length=11, polyorder=3):
    """
...
def main():
    ...
    # Design ECG filter (0.5 - 100 Hz bandpass)
    b_ecg, a_ecg = design_ecg_filter(fs=SAMPLING_RATE, lowcut=0.5, highcut=100, order=4)
    print(f"Filter 1 designed: Butterworth Bandpass for ECG (0.5-100 Hz, order 4) @ {SAMPLING_RATE} SPS")
    print(f"Filter 2 designed: Savitzky-Golay Smoothing (window=11, polyorder=3)")
    print("=" * 80 + "\n")
    ...
    try:
        ...
        ecg_smoothed = apply_smoothing_filter(ecg_filtered, window_length=11, polyorder=3)
        ...
        ax2.plot(ecg_time, ecg_filtered, color='red', linewidth=0.8, label='ECG (Bandpass 0.5-100 Hz)')
        ax2.set_xlabel('Time (seconds)', fontsize=11)
        ax2.set_ylabel('ECG (µV)', fontsize=11)
        ax2.set_title(f'ECG Signal - Butterworth Bandpass (0.5-100 Hz, order 4)', fontsize=12)
        ...
        ax3.set_title(f'ECG Signal - Smoothed (Savitzky-Golay: window=11, polyorder=3)', fontsize=12)
        ...
        print(f"\nECG Signal (Bandpass 0.5-100 Hz):")
...
#CODE before for original format of writing to serial
# Serial.print("ECG: ");
# Serial.print(uv);
# Serial.print(" | Count: ");
# Serial.println(dataCount);

# other fot this format0860
# 5420
# 1270
# 650
# 5180
...
# def design_ecg_filter(fs=SAMPLING_RATE, lowcut=0.5, highcut=100, order=4):
#     """
#     Design a Butterworth bandpass filter for ECG signal at 256 SPS
...
# def main():
#     print("=" * 80)
#     print(f"ECG Data Logger with Butterworth Bandpass + Savitzky-Goyal Smoothing Filters")
#     print("=" * 80)
#     print(f"Serial Port: {SERIAL_PORT} @ {BAUD_RATE} baud")
#     print(f"Sampling Rate: {SAMPLING_RATE} SPS")
#     print(f"Collection Time: {COLLECTION_TIME} seconds")
#     print("Waiting for data (raw numbers, one per line)...\n")
...
#                 if raw_line and raw_line.lstrip('-').isdigit():  # Check if it's a number (including negative)
#                     try:
#                         ecg_value = int(raw_line)
...
#         ax1.plot(ecg_time, ecg_buffer, color='blue', linewidth=0.8, label='ECG (Original)', alpha=0.7)
#         ax1.set_xlabel('Time (seconds)', fontsize=11)
#         ax1.set_ylabel('ECG (counts)', fontsize=11)
#         ax1.set_title(f'ECG Signal - Original - {len(ecg_buffer)} samples @ {actual_ecg_sps:.1f} SPS', fontsize=12)
#         ax1.grid(True, alpha=0.3)
#         ax1.legend(loc='upper right', fontsize=10)
...
#         ax2.plot(ecg_time, ecg_filtered, color='red', linewidth=0.8, label='ECG (Bandpass 0.5-100 Hz)')
#         ax2.set_xlabel('Time (seconds)', fontsize=11)
#         ax2.set_ylabel('ECG (counts)', fontsize=11)
#         ax2.set_title(f'ECG Signal - Butterworth Bandpass (0.5-100 Hz, order 4)', fontsize=12)
#         ax2.grid(True, alpha=0.3)
#         ax2.legend(loc='upper right', fontsize=10)
...
#         ax3.plot(ecg_time, ecg_smoothed, color='purple', linewidth=1.2, label='ECG (Bandpass + Savitzky-Golay)', alpha=0.9)
#         ax3.set_xlabel('Time (seconds)', fontsize=11)
#         ax3.set_ylabel('ECG (counts)', fontsize=11)
#         ax3.set_title(f'ECG Signal - Smoothed (Savitzky-Golay: window=11, polyorder=3)', fontsize=12)
#         ax3.grid(True, alpha=0.3)
#         ax3.legend(loc='upper right', fontsize=10)
...
#         print("=" * 80)
#         print(f"\nECG Signal (Original):")
#         print(f"  Min:      {ecg_min:8d} counts")
#         print(f"  Max:      {ecg_max:8d} counts")
#         print(f"  Range:    {ecg_max - ecg_min:8d} counts")
#         print(f"  Mean:     {ecg_mean:8.1f} counts")
        
#         print(f"\nECG Signal (Bandpass 0.5-100 Hz):")
#         print(f"  Min:      {ecg_filt_min:8.1f} counts")
#         print(f"  Max:      {ecg_filt_max:8.1f} counts")
#         print(f"  Range:    {ecg_filt_max - ecg_filt_min:8.1f} counts")
#         print(f"  Mean:     {ecg_filt_mean:8.1f} counts")
        
#         print(f"\nECG Signal (Bandpass + Savitzky-Golay Smoothed):")
#         print(f"  Min:      {ecg_smooth_min:8.1f} counts")
#         print(f"  Max:      {ecg_smooth_max:8.1f} counts")
#         print(f"  Range:    {ecg_smooth_max - ecg_smooth_min:8.1f} counts")
#         print(f"  Mean:     {ecg_smooth_mean:8.1f} counts")
...
#             f.write('Time (s),Original (counts),Bandpass (counts),Smoothed (counts)\n')
#             for i, t in enumerate(ecg_time):
#                 f.write(f'{t:.4f},{ecg_buffer[i]},{ecg_filtered[i]:.1f},{ecg_smoothed[i]:.1f}\n')
#         print("✓ Data saved to 'ecg_data.csv'")
